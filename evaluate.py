...
def train(args, model, dataloader, criterion, optimizer, scheduler, mixup_fn=None, search_space=None, teacher_model=None):
    ...
    for it in range(iters):
        ...
        if args.local_rank in [-1, 0] and ((it+1) % args.log_interval == 0 or it+1 == iters or it == 0):
            args.logger.info(f"Epoch {epoch:>3d} Iter {it+1:>4d}: loss={Loss.item():6.4f}")
...
def main(args):
    ...
    # prepare dataloader
    train_sampler = RandomSampler(train_set) if args.local_rank == -1 else DistributedSampler(train_set, shuffle=True, drop_last=True)
    valid_sampler = SequentialSampler(valid_set) if args.local_rank == -1 else DistributedSampler(valid_set, shuffle=False, drop_last=False)

    train_loader = DataLoader(train_set, sampler=train_sampler, **dataloader_config)
    dataloader_config["drop_last"] = False
    dataloader_config["batch_size"] = 250
    valid_loader = DataLoader(valid_set, sampler=valid_sampler, **dataloader_config)
    ...
    logger.info(f"Optimizer {colorstr(cfg.optimizer.opt)} and Scheduler {colorstr(cfg.scheduler.sched)} selected!")
    ...
    for epoch in range(1, total_epoch+1):
        ...
        if args.local_rank in [-1, 0]:
            ...
            logger.info(f"Epoch {epoch:>3d} LR: {lr:.8f} Train loss: {train_loss:.3f} Valid loss: {valid_loss:.3f} Valid acc: {valid_acc:.2%}")
            if args.wandb:
                wandb.log({
