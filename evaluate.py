...
class AutoFormerSpace:
    ...
    def mid(self, full_dict=False):
        config = {
            "embed_dim": sorted(self.search_embed_dim)[len(self.search_embed_dim)//2],
            "depth": sorted(self.search_depth)[len(self.search_depth)//2],
            "num_heads": [sorted(self.search_num_heads)[len(self.search_num_heads)//2] for _ in range(self.depth)],
            "mlp_ratio": [sorted(self.search_num_ratio)[len(self.search_num_ratio)//2] for _ in range(self.depth)]
        }
        self.config = config
        return self.to_dict(config) if full_dict else config
...
def main(args):
    ...
    config = getattr(search_space, args.subnet)()
    ...
    with torch_distributed_zero_first(args.local_rank):
        if args.local_rank in [-1, 0]:
            logger.info(f"Subnet({colorstr('magenta', args.subnet)}): {colorstr('blue', config)}. Params: {colorstr('green', str(round(params/1e6, 2))+' M'):>8s} Acc: {colorstr('green', str(round(valid_acc*100, 2)) + '%'):>8s}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subnet', type=str, required=True, help='subnet')
...
"""
CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=1 torchrun --nproc_per_node 1 --master_port 28100 evaluate.py --cfg config/imagenet-100.yaml --out ../results/in100_random_sd --subnet max
